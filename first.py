...
print("\n--- AUDIT FINDINGS ---")
print("- ca: 65.9% missing → DROP but create missingness flag")
print("- thal: 51.9% missing → DROP but create missingness flag")
print("- slope: 33% missing → KEEP, impute, create missingness flag")
...
def engineer_features(df):
    ...
    # Drop columns with >50% missing (after creating flags)
...
def build_preprocessor(X_ref):
    """Build preprocessor from a reference dataframe's column structure."""
...
neg_count = (y == 0).sum()
pos_count = (y == 1).sum()
print(f"Class balance: neg={neg_count}, pos={pos_count}, scale_pos_weight={neg_count/pos_count:.3f}")
...
models = {
    ...
    'XGB': XGBClassifier(
        ...
        reg_alpha=0.1, reg_lambda=1.0, scale_pos_weight=neg_count / pos_count,
        eval_metric='logloss', random_state=42, use_label_encoder=False
    ),
    'LGBM': LGBMClassifier(
        n_estimators=300, max_depth=3, learning_rate=0.05, num_leaves=7,
...
# Keep top 5, exclude unstable
top_models = []
for name, r in ranked:
    if r['std'] <= 0.06 and len(top_models) < 5:
...
def get_best_threshold(model_or_pipe, X, y, cv_splits=5, is_pipe=False):
    """Tune threshold inside CV. Returns (best_threshold, f1_at_default, f1_at_best, gain)."""
...
for name in top_models:
    ...
    print(f"  {name:25s}  threshold={tr['threshold']:.3f} +/- {tr['threshold_std']:.3f}  "
          f"F1@0.5={tr['f1_default']:.4f}  F1@best={tr['f1_tuned']:.4f}  gain=+{tr['gain']:.4f}")

# ============================================================
# STEP 6: OPTUNA TUNING FOR TOP 2 MODELS
# ============================================================
print("\n" + "=" * 60)
print("STEP 6: OPTUNA TUNING (top 2 models, 80 trials each)")
print("=" * 60)

# Pick top 2 by CV F1 mean
top2 = top_models[:2]
print(f"Tuning: {top2}")
...
for name in top2:
    ...
    print(f"  {name} best F1: {study.best_value:.4f}")
...
# Build diverse estimator set: tuned top 2 + untuned survivors from different families
# Ensure diversity: pick from different families
model_families = {
    'LogReg_balanced': 'linear', 'LogReg_elasticnet': 'linear',
    'RF': 'bagging', 'ExtraTrees': 'bagging',
    'GBM': 'boosting', 'XGB': 'boosting', 'LGBM': 'boosting', 'AdaBoost': 'boosting',
...
# Add remaining survivors from DIFFERENT families
for name in top_models:
    if name in top2:
        continue
...
# If we still have < 3 estimators, add next best regardless of family
...
print(f"  Stacking F1 = {stack_scores.mean():.4f} +/- {stack_scores.std():.4f}")

# Score best tuned single model
...
print(f"  Best single ({best_single_name}) F1 = {single_scores.mean():.4f} +/- {single_scores.std():.4f}")

# Decision: stacking vs single
...
if stack_advantage >= 0.005:
    ...
    print(f"  >>> Using STACKING (advantage >= 0.005)")
else:
    final_pipe = best_single_pipe
    final_name = f"Single: {best_single_name}"
    final_cv_scores = single_scores
    print(f"  >>> Using SINGLE MODEL (stacking advantage < 0.005, simpler = more stable)")

# Also compare against all untuned top models for completeness
print("\n  --- Full comparison ---")
all_candidates = [(final_name, final_cv_scores)]
for name in top_models:
    if name in top2:
        tuned_m = build_tuned_model(name, tuned_params[name])
        p = Pipeline([('prep', preprocessor), ('model', tuned_m)])
        s = cross_val_score(p, X, y, cv=cv, scoring='f1', n_jobs=-1)
        all_candidates.append((f"{name}_tuned", s))

for cname, cscores in sorted(all_candidates, key=lambda x: -x[1].mean()):
    print(f"    {cname:30s}  F1 = {cscores.mean():.4f} +/- {cscores.std():.4f}")

# ============================================================
# STEP 8: FINAL THRESHOLD TUNING
# ============================================================
print("\n" + "=" * 60)
print("STEP 8: FINAL THRESHOLD TUNING (10-fold)")
print("=" * 60)

final_thresh = get_best_threshold(final_pipe, X, y, cv_splits=10, is_pipe=True)
best_threshold = final_thresh['threshold']
print(f"  Final threshold: {best_threshold:.3f} +/- {final_thresh['threshold_std']:.3f}")
print(f"  F1 @ 0.5:    {final_thresh['f1_default']:.4f}")
print(f"  F1 @ best:   {final_thresh['f1_tuned']:.4f}")
print(f"  Gain:         +{final_thresh['gain']:.4f}")

# ============================================================
# STEP 9: TRAIN FINAL + GENERATE PREDICTIONS
# ============================================================
print("\n" + "=" * 60)
print("STEP 9: TRAIN FINAL + OUTPUTS")
print("=" * 60)
...
print(f"Saved: best_threshold.txt ({best_threshold:.3f})")

# Generate submission on the training data (since no separate test set provided)
...
# ============================================================
print("\n" + "=" * 60)
print("STEP 10: SANITY CHECKS")
print("=" * 60)

train_dist = y.value_counts(normalize=True).to_dict()
pred_dist = pd.Series(y_pred).value_counts(normalize=True).to_dict()
print(f"  Train target dist:  {train_dist}")
print(f"  Prediction dist:    {pred_dist}")

# Check for wild distribution shift
for cls in [0, 1]:
    diff = abs(train_dist.get(cls, 0) - pred_dist.get(cls, 0))
    if diff > 0.15:
        print(f"  WARNING: Class {cls} distribution shifted by {diff:.2f}!")
    else:
        print(f"  Class {cls} shift: {diff:.3f} (OK)")

assert not pd.Series(y_pred).isnull().any(), "NaN in predictions!"
assert len(y_pred) == len(df_full), "Row count mismatch!"
print(f"  No NaN in predictions: OK")
print(f"  Row count matches: OK ({len(y_pred)})")

# Results summary
with open('results_summary.txt', 'w') as f:
    f.write("HEART DISEASE PREDICTION — COMPETITION v2 RESULTS\n")
    f.write("=" * 50 + "\n\n")
    f.write(f"Final model: {final_name}\n")
    f.write(f"CV F1 (25 folds): {final_cv_scores.mean():.4f} +/- {final_cv_scores.std():.4f}\n")
    f.write(f"Optimal threshold: {best_threshold:.3f}\n")
    f.write(f"F1 @ tuned threshold: {final_thresh['f1_tuned']:.4f}\n")
    f.write(f"F1 gain from threshold: +{final_thresh['gain']:.4f}\n\n")
    f.write("=== MODEL ZOO RANKINGS ===\n")
    for i, (name, r) in enumerate(ranked):
        f.write(f"  {i+1}. {name:25s}  F1 = {r['mean']:.4f} +/- {r['std']:.4f}\n")
    f.write(f"\n=== OPTUNA TUNING ===\n")
    for name in top2:
        f.write(f"  {name}: F1={tuned_f1[name]:.4f}, params={tuned_params[name]}\n")
    f.write(f"\n=== ENSEMBLE ===\n")
    f.write(f"  Stacking F1 = {stack_scores.mean():.4f} +/- {stack_scores.std():.4f}\n")
    f.write(f"  Best single F1 = {single_scores.mean():.4f} +/- {single_scores.std():.4f}\n")
    f.write(f"  Advantage: {stack_advantage:+.4f}\n")
    f.write(f"\nPrediction distribution: {pred_dist}\n")
    f.write(f"Training distribution: {train_dist}\n")
...
print(f"  CV F1 (25 folds):   {final_cv_scores.mean():.4f} +/- {final_cv_scores.std():.4f}")
print(f"  Optimal threshold:  {best_threshold:.3f}")
print(f"  F1 @ threshold:     {final_thresh['f1_tuned']:.4f}")
print(f"  Expected test F1:   ~{final_cv_scores.mean():.4f}")
print("=" * 60)
